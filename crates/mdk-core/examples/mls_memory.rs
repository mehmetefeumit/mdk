...
fn generate_identity() -> (Keys, MDK<MdkMemoryStorage>) {
...
async fn main() -> Result<(), Error> {
    ...
    let (alice_keys, alice_mdk) = generate_identity();
    tracing::info!("Alice identity generated");
    let (bob_keys, bob_mdk) = generate_identity();
    ...
    assert_eq!(
        bob_mdk.get_messages(bob_mls_group_id, None).unwrap().len(),
        1,
    ...
    let (charlie_keys, charlie_mdk) = generate_identity();
